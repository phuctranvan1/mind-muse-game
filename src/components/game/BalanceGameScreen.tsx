...
import { BalanceState } from "@/hooks/useBalanceGame";
import DarkModeToggle from "./DarkModeToggle";
import PowerUpButtons from "./PowerUpButtons";
import WinModal from "./WinModal";
import { Progress } from "@/components/ui/progress";

const OPTION_COLORS = ["bg-tile-1","bg-tile-5","bg-tile-3","bg-tile-7"];

interface Props {
  game: BalanceState; time: string;
...
const BalanceGameScreen = ({ game, time, onAnswer, onHint, onPeek, onUndo, onRestart, onMenu, dark, onToggleDark }: Props) => {
    ...
    <div className="py-6 sm:py-8">
      <div className="flex justify-between items-start mb-5 gap-2">
        <div className="min-w-0">
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-primary">Balance</h1>
      ...
      <div className="bg-grid border-2 border-grid-border rounded-[var(--radius-outer)] p-6">
        <p className="text-xs text-muted-foreground text-center mb-3">Q {game.currentIndex + 1}/{game.problems.length}</p>
        <div className="text-center mb-4">
          <div className="text-6xl mb-2">⚖️</div>
          <div className="bg-card border border-border rounded-[var(--radius-inner)] p-4 text-sm text-foreground leading-relaxed">
            {problem.question}
          </div>
        </div>
          ...
          {problem.options.map((opt, i) => {
            ...
            return (
              <motion.button key={i} onClick={() => game.selectedAnswer === null && onAnswer(opt)}
                ...
                className={`py-3 rounded-[var(--radius-inner)] font-bold text-sm border shadow-[var(--tile-shadow)] transition-all ${colorClass} ${game.selectedAnswer !== null ? "cursor-default" : "cursor-pointer"}`}>
                {opt}
              </motion.button>
            );
          })}
