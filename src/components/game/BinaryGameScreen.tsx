...
import { BinaryState } from "@/hooks/useBinaryGame";
...
const BinaryGameScreen = ({ game, time, onAnswer, onHint, onPeek, onUndo, onRestart, onMenu, dark, onToggleDark }: Props) => {
  const problem = game.problems[game.currentIndex];
  const progress = (game.currentIndex / game.problems.length) * 100;
  const baseLabel = problem.targetBase === "bin" ? "Binary" : problem.targetBase === "hex" ? "Hex" : "Decimal";

  return (
    <div className="py-6 sm:py-8">
      <div className="flex justify-between items-start mb-5 gap-2">
        <div className="min-w-0">
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-primary">Binary Quiz</h1>
      ...
      <div className="bg-grid border-2 border-grid-border rounded-[var(--radius-outer)] p-6">
        <p className="text-xs text-muted-foreground text-center mb-3">Question {game.currentIndex + 1} of {game.problems.length} — Convert to {baseLabel}</p>
        <div className="text-center mb-6">
          <div className="inline-block px-6 py-4 rounded-[var(--radius-inner)] bg-card border-2 border-primary/30 font-mono font-bold text-2xl text-primary">{problem.source}</div>
          <p className="text-sm text-muted-foreground mt-2">= ? ({baseLabel})</p>
        </div>
          ...
          {problem.options.map((opt, i) => {
            ...
            return (
              <motion.button key={i} onClick={() => game.selectedAnswer === null && onAnswer(opt)}
                ...
                className={`py-4 rounded-[var(--radius-inner)] font-bold font-mono text-base border shadow-[var(--tile-shadow)] transition-all ${colorClass} ${game.selectedAnswer !== null ? "cursor-default" : "cursor-pointer"}`}>
                {opt}
              </motion.button>
            );
          })}
...
export default BinaryGameScreen;
