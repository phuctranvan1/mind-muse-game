...
import { CipherState } from "@/hooks/useCipherGame";
import DarkModeToggle from "./DarkModeToggle";
import PowerUpButtons from "./PowerUpButtons";
import WinModal from "./WinModal";
import { Progress } from "@/components/ui/progress";

const OPTION_COLORS = ["bg-tile-5","bg-tile-3","bg-tile-7","bg-tile-1"];

interface Props {
  game: CipherState; time: string;
...
const CipherGameScreen = ({ game, time, onAnswer, onHint, onPeek, onUndo, onRestart, onMenu, dark, onToggleDark }: Props) => {
    ...
    <div className="py-6 sm:py-8">
      <div className="flex justify-between items-start mb-5 gap-2">
        <div className="min-w-0">
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-primary">Caesar Cipher</h1>
      ...
      <div className="bg-grid border-2 border-grid-border rounded-[var(--radius-outer)] p-6">
        <p className="text-xs text-muted-foreground text-center mb-2">Q {game.currentIndex + 1}/{game.problems.length} — What does this say?</p>
        <div className="text-center mb-6">
          <div className="inline-block px-6 py-4 rounded-[var(--radius-inner)] bg-card border-2 border-primary/30 font-mono font-bold text-2xl text-primary tracking-[0.3em]">
            {problem.ciphertext}
          </div>
          <p className="text-xs text-muted-foreground mt-2">🔐 Caesar-encrypted · select the correct decryption</p>
        </div>
          ...
          {problem.options.map((opt, i) => {
            ...
            return (
              <motion.button key={i} onClick={() => game.selectedAnswer === null && onAnswer(opt)}
                ...
                className={`py-3 px-2 rounded-[var(--radius-inner)] font-semibold font-mono text-sm border shadow-[var(--tile-shadow)] transition-all ${colorClass} ${game.selectedAnswer !== null ? "cursor-default" : "cursor-pointer"}`}>
                {opt}
              </motion.button>
            );
          })}
