...
import { EmojiMathState } from "@/hooks/useEmojiMathGame";
...
interface Props {
  game: EmojiMathState; time: string;
...
const EmojiMathGameScreen = ({ game, time, onAnswer, onHint, onPeek, onUndo, onRestart, onMenu, dark, onToggleDark }: Props) => {
    ...
    <div className="py-6 sm:py-8">
      <div className="flex justify-between items-start mb-5 gap-2">
        <div className="min-w-0">
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-primary">Emoji Math</h1>
      ...
      <div className="bg-grid border-2 border-grid-border rounded-[var(--radius-outer)] p-6">
        <p className="text-xs text-muted-foreground text-center mb-4">Q {game.currentIndex + 1}/{game.problems.length} — What is the answer?</p>
        <div className="bg-card border border-border rounded-[var(--radius-inner)] p-4 mb-4">
          {problem.clues.map((clue, i) => (
            <p key={i} className="text-center text-2xl font-semibold mb-2 last:mb-0">{clue}</p>
          ))}
        </div>
        <div className="text-center mb-5">
          <span className="inline-block px-6 py-3 rounded-[var(--radius-inner)] bg-primary/10 border border-primary/30 text-2xl font-bold text-primary">
            {problem.question}
          </span>
        </div>
          ...
          {problem.options.map((opt, i) => {
            ...
            return (
              <motion.button key={i} onClick={() => game.selectedAnswer === null && onAnswer(opt)}
                ...
                className={`py-4 px-3 rounded-[var(--radius-inner)] font-bold text-xl border shadow-[var(--tile-shadow)] transition-all text-center ${colorClass} ${game.selectedAnswer !== null ? "cursor-default" : "cursor-pointer"}`}>
                {opt}
              </motion.button>
            );
          })}
...
export default EmojiMathGameScreen;
