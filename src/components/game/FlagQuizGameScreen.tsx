...
import { FlagQuizState } from "@/hooks/useFlagQuizGame";
import DarkModeToggle from "./DarkModeToggle";
import PowerUpButtons from "./PowerUpButtons";
import WinModal from "./WinModal";
import { Progress } from "@/components/ui/progress";

const OPTION_COLORS = ["bg-tile-5","bg-tile-1","bg-tile-3","bg-tile-7"];

interface Props {
  game: FlagQuizState; time: string;
...
const FlagQuizGameScreen = ({ game, time, onAnswer, onHint, onPeek, onUndo, onRestart, onMenu, dark, onToggleDark }: Props) => {
    ...
    <div className="py-6 sm:py-8">
      <div className="flex justify-between items-start mb-5 gap-2">
        <div className="min-w-0">
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-primary">Flag Quiz</h1>
      ...
      <div className="bg-grid border-2 border-grid-border rounded-[var(--radius-outer)] p-6">
        <p className="text-xs text-muted-foreground text-center mb-4">Q {game.currentIndex + 1}/{game.questions.length} — Which country is this flag?</p>
        <div className="flex justify-center items-center mb-5">
          <span className="text-9xl leading-none select-none">{q.flag}</span>
        </div>
        {game.peeking && <p className="text-center text-accent font-bold mb-3 animate-pulse">👁 Answer: {q.answer}</p>}
...
export default FlagQuizGameScreen;
