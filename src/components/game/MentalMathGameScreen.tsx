...
import { MentalMathState } from "@/hooks/useMentalMathGame";
import DarkModeToggle from "./DarkModeToggle";
import PowerUpButtons from "./PowerUpButtons";
import WinModal from "./WinModal";
import { Progress } from "@/components/ui/progress";

const OPTION_COLORS = ["bg-tile-4","bg-tile-6","bg-tile-3","bg-tile-1"];

interface Props {
  game: MentalMathState; time: string;
...
const MentalMathGameScreen = ({ game, time, onAnswer, onHint, onPeek, onUndo, onRestart, onMenu, dark, onToggleDark }: Props) => {
    ...
    <div className="py-6 sm:py-8">
      <div className="flex justify-between items-start mb-5 gap-2">
        <div className="min-w-0">
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-primary">Mental Math</h1>
      ...
      <div className="bg-grid border-2 border-grid-border rounded-[var(--radius-outer)] p-6">
        <p className="text-xs text-muted-foreground text-center mb-3">Q {game.currentIndex + 1}/{game.problems.length} — What is the final result?</p>
        <div className="flex flex-wrap items-center justify-center gap-2 mb-6">
          <div className="px-4 py-2 rounded-[var(--radius-inner)] bg-card border border-primary/30 font-mono font-bold text-lg text-primary">{problem.start}</div>
          {problem.steps.map((step, i) => (
            <div key={i} className="flex items-center gap-2">
              <span className="text-muted-foreground font-bold">→</span>
              <div className="px-4 py-2 rounded-[var(--radius-inner)] bg-card border border-border font-mono font-bold text-lg text-foreground">
                {step.op} {step.val}
              </div>
            </div>
          ))}
          <div className="flex items-center gap-2">
            <span className="text-muted-foreground font-bold">→</span>
            <div className="px-4 py-2 rounded-[var(--radius-inner)] border-2 border-dashed border-primary/60 bg-primary/8 font-mono font-bold text-xl text-primary">?</div>
          </div>
        </div>
          ...
          {problem.options.map((opt, i) => {
            ...
            return (
              <motion.button key={i} onClick={() => game.selectedAnswer === null && onAnswer(opt)}
                ...
                className={`py-4 rounded-[var(--radius-inner)] font-bold font-mono text-lg border shadow-[var(--tile-shadow)] transition-all ${colorClass} ${game.selectedAnswer !== null ? "cursor-default" : "cursor-pointer"}`}>
                {opt}
              </motion.button>
            );
          })}
...
export default MentalMathGameScreen;
