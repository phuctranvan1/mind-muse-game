...
const SudokuGameScreen = ({ game, time, onSelectCell, onEnterNumber, onClear, onRestart, onMenu, dark, onToggleDark }: Props) => {
  ...
  return (
    <div className="py-8">
      <div className="flex justify-between items-end mb-6">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-primary">Sudoku</h1>
        ...
        <div className="flex items-end gap-4">
          <div className="flex gap-6">
            ...
            <div>
              ...
              <span className={`tabular-nums font-semibold text-lg ${game.mistakes > 0 ? "text-destructive" : "text-foreground"}`}>{game.mistakes}</span>
            </div>
          </div>
        </div>
            ...
            row.map((val, ci) => {
              ...
              return (
                <button
                  ...
                  } ${isWrong ? "text-destructive" : isFixed ? "text-foreground" : "text-primary"}`}
                >
                  {val !== 0 ? val : ""}
                </button>
              );
