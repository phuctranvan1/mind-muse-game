...
const CONFIGS: Record<Difficulty, { size: number; removeCount: number }> = {
  easy: { size: 4, removeCount: 4 },
  medium: { size: 4, removeCount: 8 },
  hard: { size: 9, removeCount: 35 },
  expert: { size: 9, removeCount: 45 },
  master: { size: 9, removeCount: 52 },
  grandmaster: { size: 9, removeCount: 56 },
  genius: { size: 9, removeCount: 60 },
};
...
export function useSudokuGame() {
  ...
  const restart = useCallback(() => {
    setGame(prev => {
      if (!prev) return null;
      return { ...prev, playerGrid: prev.puzzle.map(r => [...r]), mistakes: 0, won: false, selectedCell: null };
    });
  }, []);

  const goToMenu = useCallback(() => setGame(null), []);

  return { game, startGame, selectCell, enterNumber, clearCell, restart, goToMenu };
}
